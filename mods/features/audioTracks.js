...
export const LANGUAGE_CODES = ['ru', 'en', 'kk', 'de', 'fr', 'es', 'it', 'zh', 'ja', 'ko', 'ar', 'lt', 'lv', 'uk', 'pt', 'tr', 'hi'];
...
export const languageName = code => LANGUAGE_CODES.includes(code)
    ? audioText('language.' + code) : code || audioText('languageNotSpecified');
...
export async function selectYouTubeTrack(id, { isCurrent = () => true, delay = ms => new Promise(r => setTimeout(r, ms)) } = {}) {
    ...
    if (!track) throw new Error(audioText('thisTrackIsNoLongerAvailable'));
    if (inventory.selected?.id === id) return;
    if (!inventory.canSelect) throw new Error(audioText('youtubeDidNotProvideAudioTrack'));
    if (!isCurrent()) throw new Error(audioText('trackSelectionCancelled'));
    ...
    if (!descriptor) throw new Error(audioText('youtubeHasNotMadeThisTrack'));
    ...
    throw new Error(audioText('youtubeDidNotConfirmTheTrack'));
}

export async function ensureOriginal(options) {
    const inventory = audioInventory();
    if (options?.isCurrent && !options.isCurrent()) throw new Error(audioText('trackSelectionCancelled'));
    ...
    throw new Error(audioText('youtubeHasNotIdentifiedTheOriginal'));
}
